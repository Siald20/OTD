...
/// </summary>
public readonly record struct InputInfo(int InputNumber, string InputName, InputState State);

/// <summary>
///     Event args raised when a sensor changes state.
/// </summary>
public sealed class InputStateChangedEventArgs(
    Guid providerUid,
    InputInfo inputInfo) : EventArgs
{
    ...
    /// </summary>
    public int InputNumber { get; } = inputInfo.InputNumber;

    /// <summary>
    ///     Human-readable sensor label for orientation (e.g. "1.2").
    /// </summary>
    public string InputName { get; } = inputInfo.InputName;

    /// <summary>New state of the sensor.</summary>
    public InputState State { get; } = inputInfo.State;
}