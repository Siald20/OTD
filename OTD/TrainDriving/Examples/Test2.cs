...
public class Test2
{
    ...
    private static async Task RunAsync(CommandStation commandStation, Feedback feedbackModule)
    {
        ...
        try
        {
            ...
            // 2) 5 Sekunden warten.
            Console.WriteLine("[Test2] 5 s warten...");
            ...
            // 4) Route B2 -> K102 erstellen (224cm, 40 km/h).
            var b2ToK102 = AddRoute("B2", "K102", 224, 40,
                new SensorSpec(28, 3),
                new SensorSpec(31, 43),
                new SensorSpec(30, 72));
            RebuildRuntime();

            // 5) Fahrweg Richtung K102 -> H41 stellen.
            Console.WriteLine("[Fahrweg] Richtung H41: W104=diverging");
            await _turnoutW104.SetStateAsync("diverging", cts.Token);

            // 7) Route K102 -> H41 ergänzen (163cm, 40 km/h).
            var k102ToH41 = AddRoute("K102", "H41", 163, 60,
                new SensorSpec(148, 10),
                new SensorSpec(147, 53));
            RebuildRuntime();
            
            // 6) 5 Sekunden warten, dann bei B2 abfahren.
            Console.WriteLine("[Test2] Fahrt B2 -> K102");
//            await Task.Delay(TimeSpan.FromSeconds(5), cts.Token);
            await DriveDistanceAsync(targetSpeedKmh: 60, distanceCm: b2ToK102.DistanceCm, runtime, cts.Token);

            Console.WriteLine("[Test2] Fahrt K102 -> H41");
            await DriveDistanceAsync(targetSpeedKmh: 0, distanceCm: k102ToH41.DistanceCm, runtime, cts.Token);

            Console.WriteLine("[Test2] Zug sollte bei K102 stoppen.");

            await _trainBr193.SetSpeedVAsync(0, cts.Token);
