...
//                          throw new InvalidOperationException("Train BR193 is not initialized.");
			...
			// Zug abfahren lassen
			const int vTest = 40; // Test-Geschwindigkeit in km/h
			var trainDriving = new TrainDriving(_testTrain)
			{
				...
				BrakeCurveFidelityPercent = 60.0
			};

			LogTimed("TrainDriving",
				$"Adaptive Modell aktiv: min={trainDriving.MinSpeedStepInterval.TotalMilliseconds:F0} ms, " +
				$"max={trainDriving.MaxSpeedStepInterval.TotalMilliseconds:F0} ms, " +
				$"brakePointCorrection={trainDriving.BrakePointCorrectionPercent:F1}% + " +
				$"speedDependent={trainDriving.BrakePointCorrectionPercentPerVMax:F1}% @ VMax, " +
				$"fidelity={trainDriving.BrakeCurveFidelityPercent:F0}%");

			// // Prototyp: Fahrzyklen werden ueber Signal-zu-Signal ausgewertet.
			// var route = RouteModelPrototype.CreateRouteTable();
			// _activeRouteRuntime = new RouteRuntime(route);
			// var cycleTick = _activeRouteRuntime.ApplyStep(deltaCm: 0, trajectorySpeedKmh: VTest);
			//
			// if (cycleTick.ActiveCycle is { } cycle)
			// {
			//     Console.WriteLine(
			//         $"[RouteRuntime] Verfuegbarer Zyklus {cycle.FromWaypoint.WaypointId} -> {cycle.ToWaypoint.WaypointId}, " +
			//         $"Distanz={cycle.DistanceCm:F0}cm, Vmax={cycle.AllowedSpeedKmh} km/h, " +
			//         $"BrakePreset={cycle.DriveProfile?.BrakingPreset}");
			// }
			//
			await trainDriving.AccelerateAsync(
				targetSpeed: vTest,
				cancellationToken: emergencyHotkeyCts.Token);
			//
			// const double firstLegDistanceCm = 100;
			// var postDepartureTick = _activeRouteRuntime.ApplyStep(deltaCm: firstLegDistanceCm, trajectorySpeedKmh: VTest);
			// Console.WriteLine(
			//     $"[RouteRuntime] Position nach Anfahrt: {postDepartureTick.EstimatedPositionCm:F1} cm, " +
			//     $"Waypoint={postDepartureTick.PermissionState.SourceWaypointId}, Vmax={postDepartureTick.PermissionState.MaxSpeedKmh}");
			//
			//await _trainBr193.SetSpeedVAsync(VTest, emergencyHotkeyCts.Token);
			...
			await WaitForSensorStateAsync(_feedbackModule, 148, RailSensorState.Active, emergencyHotkeyCts.Token);
			LogTimed("Feedback", "Sensor 56 aktiv -> starte Bremsrampe auf 0 km/h ueber 200 cm.");

			// var brakingTick = postDepartureTick;
			//
			// if (brakingTick.ActiveCycle is { } brakingCycle)
			// {
			//     Console.WriteLine(
			//         $"[RouteRuntime] Fahre Zyklus {brakingCycle.FromWaypoint.WaypointId} -> {brakingCycle.ToWaypoint.WaypointId}, " +
			//         $"Distanz={brakingCycle.DistanceCm:F0}cm, Vmax={brakingCycle.AllowedSpeedKmh} km/h, " +
			//         $"BrakePreset={brakingCycle.DriveProfile?.BrakingPreset}");
			//
			//     await trainDriving.DriveRouteCycleAsync(
			//         currentSpeedKmhPrototype: VTest,
			//         cycle: brakingCycle,
			//         cancellationToken: emergencyHotkeyCts.Token);
			// }
			// else
			{
				...
				LogTimed("TrainDriving", "Sende Bremsbefehl: BrakeAsync(targetSpeed=0, distance=200)");
				await trainDriving.BrakeAsync(
					targetSpeed: 0,
					distance: 153, // 153, 110, 200
					cancellationToken: emergencyHotkeyCts.Token);
			}
