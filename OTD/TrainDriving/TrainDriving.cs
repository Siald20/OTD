...
public class TrainDriving
{
    ...
    private const double DefaultBrakeCurveFidelityPercent = 60.0;
    ...
    /// <summary>
    /// Optional diagnostics event emitted for adaptive speed-step scheduling and command dispatch.
    /// </summary>
    ...
    /// <summary>
    /// Bremskurventreue in Prozent (0..100) fuer die dynamische Intervallverkuerzung.
    /// 0 = keine Dynamisierung (immer MaxSpeedStepInterval),
    /// 100 = maximale Annaeherung an die Kurve (bis min. MinSpeedStepInterval).
    /// </summary>
    public double BrakeCurveFidelityPercent { get; set; } = DefaultBrakeCurveFidelityPercent;
    ...
    /// <summary>
    /// Enables console diagnostics for adaptive speed-step scheduling.
    /// </summary>
    public bool DiagnosticLogging { get; set; }

    /// <summary>
    /// If enabled, emits loop-level diagnostic messages for every tick.
    /// </summary>
    public bool VerboseDiagnosticLogging { get; set; }
        ...
        int? lastSentCommandedSpeedRoundedKmh)
    {
        ...
        var fidelity = Math.Clamp(BrakeCurveFidelityPercent, 0.0, 100.0) / 100.0;
        if (fidelity <= 0.0)
            return maxInterval;
    ...
    {
        if (!DiagnosticLogging && DiagnosticLog is null)
            return;

        var formatted = $"[TrainDriving Diag] {DateTimeOffset.Now:HH:mm:ss.fff} {message}";
        if (DiagnosticLogging)
            Console.WriteLine(formatted);

        DiagnosticLog?.Invoke(formatted);
    }
    ...
    private void RaiseProgressTick(TrainDrivingProgressTick tick)
    {
        ...
        foreach (var subscribedDelegate in handlers.GetInvocationList())
        {
            ...
            {
                Debug.WriteLine($"[TrainDriving] ProgressTick handler failed: {ex.Message}");
            }
        }
    }
